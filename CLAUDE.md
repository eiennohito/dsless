...
- **Bug response protocol**: When diagnosing a problem, always present two options: (1) a minimal fix for the immediate symptom, and (2) a remodeling approach that eliminates the bug class. Never jump straight into a fix. Bugs are modeling signals — the minimal fix is sometimes right, but the user decides, not the agent. When remodeling, be destructive — agents default to preserving too much of the old structure. The first remodel usually carries baggage from the old code; remodel the remodel. Existing code is not precious.
